...
class TestMethods(unittest.TestCase):
    ...
    def testDoc(self):
        try:
            tables = extract_tables(path_to_directory_of_this_file + "/test.docx", debug=debug)
    ...
    def testFormats(self):
        for _format in ("csv", "tsv", "xlsx"):
            try:
                tables = extract_tables(path_to_directory_of_this_file + "/test." + _format, debug=debug)
                self.assertEqual(len(tables), 1)
    ...
    def testHTML(self):
        try:
            ...
            tables = extract_tables(html, debug=False)
            self.assertEqual(len(tables), 1)
    ...
    def testWebpage(self):
        tables = extract_tables("http://www.nuforc.org/webreports/ndxlAK.html", debug=False)
        try:
            self.assertEqual(len(tables), 1)
            self.assertTrue(len(tables[0]) > 50)
        except Exception as e:
